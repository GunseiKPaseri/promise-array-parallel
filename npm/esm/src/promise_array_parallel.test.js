...
import { assert, assertEquals, assertRejects, } from "../deps/deno.land/std@0.160.0/testing/asserts.js";
...
dntShim.Deno.test("Rejected, allSettled not throw error", async () => {
    ...
    assertEquals(allSettled, [...new Array(100)]
        .map((_, i) => (i === 50
        ? {
            status: "rejected",
...
dntShim.Deno.test("maintain sequence", async () => {
    ...
    const x = await PromiseArray
        ...
        .all();
    assertEquals(x, [...new Array(100)].map((_, i) => i * 7));
});
